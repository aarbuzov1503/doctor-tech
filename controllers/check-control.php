<?php 
session_start(); // инициализция механизма сессии
// контрольный вывод передаваемой информации
// echo "<pre>";
// 	print_r($_POST);
// echo "</pre>";


// подключаемся к базе данных

  $host = '127.0.0.1';
    $db   = '#';
    $user = '#';  // нужно изменить по необходимости
    $pass = '#';      // нужно изменить по необходимости
    $charset = 'utf8';
    $dsn = "mysql:host=$host;dbname=$db;charset=$charset";
     
// подключение и создание объекта PDO
    try {
        $pdo = new PDO($dsn, $user, $pass);
        } 
    catch (PDOException $e) {
        die('Подключение не удалось: ' . $e->getMessage());
        } 

// извлекаем логин и  пароль для проверки
$tmp_locic = false; // флаг 
$sql_select = "SELECT * FROM `users` LIMIT 0, 50 ";
$stmt = $pdo->query($sql_select);
while ($row = $stmt->fetch())
{

// проверяем есть в базе данных пользователь с данным логином и паролем
    if (($_POST['username'] == $row['username'])&&($_POST['password'] == $row['password']))
    {
    	echo "Здравствуйте";
    	echo " ";
    	echo $row['username'];
    	echo " ";
    	echo "<br/> ";
    	
    	$tmp_locic = true; // устанавливаем флаг, пользователь найден

        // необходимые данные из бд сохраняем в сессии
        // для того чтобы использовать их на страницах сайта
        $_SESSION['auth'] = $tmp_locic;
        $_SESSION['id'] = $row['id'];	
        $_SESSION['username'] = $row['username']; 
        
        
       	break;	// выходим сразу из цикла
    	
    	
    }
     
} // while

if ($tmp_locic == false) // если пользователь не найден 
     {
    	echo "Пользователь с таким Логином и Паролем не сущетсвует";
        ?>    
        
        <?php
    $timer = 5; // начальное значение таймера
?>

<p>Через <span id="timer"><?php echo $timer; ?></span> секунд будет произведено перенаправление на страницу регистрации пользователей</p>

<script>
  var timer = <?php echo $timer; ?>;
  var timerElement = document.getElementById('timer'); // элемент, содержащий значение таймера

  function countdown() {
    timer--;

    // обновляем значение таймера на странице
    timerElement.textContent = timer;

    if (timer <= 0) {
      window.location = '../users.php'; // перенаправляем на страницу регистрации пользователей
    } else {
      setTimeout(countdown, 1000); // обновляем таймер каждую секунду
    }
  }

  // запускаем таймер при загрузке страницы
  setTimeout(countdown, 1000);
</script>

        
    <!--<p>Через 5 секунд будет произведено перенаправление на страницу регистрации пользователей</p>-->
   <!--<script> window.setTimeout(function() { window.location = '../users.php'; }, 5000) </script>-->
<?php
       }


// сделаем редирект (переход) на страницу пользователя

if (!empty($_SESSION['id'])) { 
?>    
    <!--<p>Через 5 секунд будет произведено перенаправление в личный кабинет</p>-->
   <!--<script> window.setTimeout(function() { window.location = '../lk.php'; }, 5000) </script>-->
           <?php
    $timer = 5; // начальное значение таймера
?>

<p>Через <span id="timer"><?php echo $timer; ?></span> секунд будет произведено перенаправление на страницу личного кабинета</p>

<script>
  var timer = <?php echo $timer; ?>;
  var timerElement = document.getElementById('timer'); // элемент, содержащий значение таймера

  function countdown() {
    timer--;

    // обновляем значение таймера на странице
    timerElement.textContent = timer;

    if (timer <= 0) {
      window.location = '../lk.php'; // перенаправляем на страницу личного кабинета
    } else {
      setTimeout(countdown, 1000); // обновляем таймер каждую секунду
    }
  }

  // запускаем таймер при загрузке страницы
  setTimeout(countdown, 1000);
</script>
<?php
} 


?>